  ...
  constructor(private graph: GraphService) {
    this.graph.getDocuments().then((data: any) => {
      ...
      for (let f of data?.d?.Files?.results) {
        let url = `https://loquesea.sharepoint.com/:w:/r/_layouts/15/WopiFrame.aspx?sourcedoc=%7B${f.UniqueId.toUpperCase()}%7D&file=${encodeURI(f.Name)}&action=default&mobileredirect=true`;
